...
public:
	...
	bool OnUserUpdate(float fElapsedTime) override
	{
		...
		if (IsConnected())
		{
			while (!Incoming().empty())
			{
				...
				case(GameMsg::Client_Accepted):
					{
						std::cout << "Server accepted client - you're!\n";
		...
		{
			//where will be the worst case?
			olc::vf2d vPotentialPosition = object.second.vPos + object.second.vVel * fElapsedTime;

			//extract region of the world cells that could have collision this frame
			...
			// iterate through each cell in the test area
			olc::vi2d vCell;
			for (vCell.y = vAreaTL.y; vCell.y <= vAreaBR.y; vCell.y++)
			{
				for (vCell.x = vAreaTL.x; vCell.x <= vAreaBR.x; vCell.x++)
				{
					...
					{
						// .. it is! so work out the nearest point to future player position, around the perimeter
						// of cell rectangle. We can test the distance to this point to see if we have collided
						...
						//but modified to actually works
						olc::vf2d vRayToNearest = vNearestPoint - vPotentialPosition;
						float fOverlap = object.second.fRadius - vRayToNearest.mag();
						if (std::isnan(fOverlap)) fOverlap = 0;

						//if overlap is positive, then a collision has occured, so we displace backwards
						//by the overlap amount. The position is then tested against other tiles in the area
