...
protected:
	...
	{
		//client passed validation check, so send a message informing
	...
	void OnClientDisconnect(std::shared_ptr<olc::net::connection<GameMsg>> client) override
	{
		if (client)
		{
			...
			{
				//client never added to the roster, so just let it disapear
			}
			else
			{
				auto& pd = m_mapPlayerRoster[client->GetID()];
				std::cout << "[UNGRACEFULL REMOVAL]:" + std::to_string(pd.nUniqueID) + "\n";
	...
	void OnMessage(std::shared_ptr<olc::net::connection<GameMsg>> client, olc::net::message<GameMsg>& msg) override
	{
		...
		{
			//simply bounce the update to everyone except incoming client
